...
class AdminSelectedProgress extends StatefulWidget {
  ...
  @override
  _AdminSelectedProgressState createState() => _AdminSelectedProgressState();
}

class _AdminSelectedProgressState extends State<AdminSelectedProgress> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: sessions.isEmpty
          ? Center(
              child:
                  CircularProgressIndicator()) // Show loading indicator if sessions are being fetched
          : OrientationBuilder(
    ...
    return sessions.asMap().entries.map((entry) {
      ...
      if (session.sessionList != null && session.sessionList!.isNotEmpty) {
        ...
        if (sess.aSess != null && sess.aSess!.length >= 3) {
          judgementCall =
              sess.aSess![2] ?? ''; // Accessing the first string in aSess
          feedback =
              sess.aSess![1] ?? ''; // Accessing the second string in aSess
        ...
        ]); // Handle the case when sessionList is empty
      }
    }).toList();
  }
}
